...
public final class SearchEnginer {
    ...
    private SearchEnginer(String indexDir) {
        try {
            ...
            /* 默认使用最细粒度分词 */
            analyzer = TKAnalyzerTool.getInstance().getAnalyzer();
    ...
    /**
     * 此方法返回切换Directory<br>
     * 要关闭复合文件格式(除段信息文件，锁文件，以及删除的文件外，其他的一系列索引文件压缩一个后缀名为cfs的文件,默认为关闭) 生成复合文件将消耗更多的时间,但它有更好的查询效率,适合查询多更新少的场合<br>
    ...
    public void searchHighlight(QueryBuilder builder, PageModel<MapContainer> model, Set<String> fields) {
        ...
        try {
            nmrt.waitForGeneration(reopenToken);
            searcher = manager.acquire();
            TopDocs docs = searcher.search(builder.getQuery(), builder.getFilter(), 5000);
            model.setTotalCount(docs.totalHits);
