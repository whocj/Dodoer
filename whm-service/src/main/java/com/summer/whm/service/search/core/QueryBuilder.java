...
public class QueryBuilder {
    ...
    public QueryBuilder addMust(String field, String value) {
        ...
        // 当字段author,type,objId,id
        if (DocType.DOC_FIELD_AUTHOR.equals(field) || DocType.DOC_FIELD_TYPE.equals(field)
                || DocType.DOC_FIELD_OBJID.equals(field) || DocType.DOC_FIELD_ID.equals(field)) {
            must.add(new Term(field, value.toLowerCase()));
        } else {
    ...
    public Query getQuery() {
        ...
        if(must != null && must.size() > 0){
            BooleanQuery queryBan = new BooleanQuery();
            for (Term term : must) {
                queryBan.add(new TermQuery(term), Occur.MUST);
            }
            
            if(should != null && should.size() > 0){
                queryBan.add(query, Occur.MUST);
            }
            return queryBan;
        }
