...
import com.summer.whm.spider.utils.domain.DomainSuffix;
import com.summer.whm.spider.utils.domain.DomainSuffixes;

/** Utility class for URL analysis */
public class URLUtil {

    /**
     * Resolve relative URL-s and fix a few java.net.URL errors in handling of URLs with embedded params and pure query
     * targets.
     * 
     * @param base base url
    ...
    public static URL resolveURL(URL base, String target) throws MalformedURLException {
        ...
        /*
         * this is probably not needed anymore - see NUTCH-797. // handle params that are embedded into the base url -
         * move them to target // so URL class constructs the new url class properly if (base.toString().indexOf(';') >
     ...
     * <p>
     * If the taget contains params (i.e. ';xxxx') information then the target params information is assumed to be
     * correct and any base params information is ignored. If the base contains params information but the tareget does
     * not, then the params information is moved to the target allowing it to be correctly determined by the
     * java.net.URL class.
     * 
     ...
     */
    public static URL fixEmbeddedParams(URL base, String target) throws MalformedURLException {
    ...
    /**
     * Returns the domain name of the url. The domain name of a url is the substring of the url's hostname, w/o
     * subdomain names. As an example <br>
     * <code>
     ...
     * 
     * <p>
     * This method implements an extended version of the algorithm used by the Yahoo! Slurp crawler described here:<br>
     * <a href= "http://help.yahoo.com/l/nz/yahooxtra/search/webcrawler/slurp-11.html"> How does the Yahoo! webcrawler
     * handle redirects?</a> <br>
     * <br>
     * <ol>
     * <li>Choose target url if either url is malformed.</li>
     * <li>If different domains the keep the destination whether or not the redirect is temp or perm</li>
     * <ul>
     * <li>a.com -> b.com*</li>
     * </ul>
     ...
     * </ul>
     * <li>If the redirect is permanent and the source is not root and the destination is root, keep the destination</li>
     * <ul>
     * <li>a.com/xyz/index.html -> a.com*</li>
     * </ul>
     * <li>If the redirect is permanent and neither the source nor the destination is root, then keep the destination</li>
     * <ul>
     * <li>a.com/xyz/index.html -> a.com/abc/page.html*</li>
     * </ul>
     * <li>If the redirect is temporary and source is root and destination is not root, then keep the source</li>
     * <ul>
     * <li>*a.com -> a.com/xyz/index.html</li>
     * </ul>
     * <li>If the redirect is temporary and source is not root and destination is root, then keep the destination</li>
     * <ul>
     * <li>a.com/xyz/index.html -> a.com*</li>
     * </ul>
     * <li>If the redirect is temporary and neither the source or the destination is root, then keep the shortest url.
     * First check for the shortest host, and if both are equal then check by path. Path is first by length then by the
     * number of / path separators.</li>
     * <ul>
     * <li>a.com/xyz/index.html -> a.com/abc/page.html*</li>
     * <li>*www.a.com/xyz/index.html -> www.news.a.com/xyz/index.html</li>
     * </ul>
     * <li>If the redirect is temporary and both the source and the destination are root, then keep the shortest
     * sub-domain</li>
     * <ul>
     * <li>*www.a.com -> www.news.a.com</li>
     * </ul>
     * <br>
     * While not in this logic there is a further piece of representative url logic that occurs during indexing and
     * after scoring. During creation of the basic fields before indexing, if a url has a representative url stored we
     * check both the url and its representative url (which should never be the same) against their linkrank scores and
     * the highest scoring one is kept as the url and the lower scoring one is held as the orig url inside of the index.
     * 
    ...
    /**
     * Returns the page for the url. The page consists of the protocol, host, and path, but does not include the query
     * string. The host is lowercased but the path is not.
     * 
     * @param url The url to check.
    ...
    public static boolean compile(String host, String host2) {
        if (host != null && host2 != null) {
            try{
                if (getFirstDomain(host).equals(getFirstDomain(host2))) {
                    return true;
                }
            }catch(Exception e){
                return false;
    ...
    public static void main(String[] args) throws IOException {
        String url = "http://www.cnblogs.com/vamei/archive/2013/02/21/2918069.html";
        System.out.println(URLUtil.getHost(url));

        String u1 = "111";
        String u2 = "11";
        String[] strs = u1.split("\\.");
        System.out.println(strs[0]);
        System.out.println(compile(u1, u2));
    }
}
