...
public class ParseStoryScriptTask implements Runnable {
    ...
    public ParseStoryScriptTask(SpiderContext spiderContext, StoryInfoService storyInfoService,
            ...
            SpiderStoryJobService spiderStoryJobService, Map<Integer, String> categoryMap) {
    ...
    public void parseStoryJob(ParseStoryElement parseStoryElement) throws InterruptedException {
        ...
        map.put(SITE_CATEGORY_KEY,
                SpiderSiteCategoryService.getCategoryMapByTemplateId(tempSpiderStoryJob.getTemplateId()));
    ...
    public void parseInfo(ParseStoryElement parseStoryElement, int current, List<String> list)
        ...
        if (current == 1) {// 第一页
            if (spiderStoryJob.getStoryId() == null) {// 第一次抓取数据，需要抓取文章基本信息
                ...
                storyInfo.setCrawlUrl(parseStoryElement.getHtmlPage().getWebResponse().getRequestSettings().getUrl()
                        .toString());
                ...
                Integer categoryId = (Integer) ScriptManager.getInstance().run(template.getCategoryXPath(), map);// 处理标题
                if (storyInfo.getCategoryId() == null && categoryId != null) {
                    storyInfo.setCategoryId(categoryId);
                }
                ...
                if (picPath != null) {
                    //上传图片到图片服务器
                    uploadPic(storyInfo.getId(), picPath);
                }
                ...
                if (storyInfo.getPicPath() == null || "/RES/images/default_120_150.jpg".equals(storyInfo.getPicPath())) {
                    String picPath = (String) ScriptManager.getInstance().run(template.getPicPathXPath(), map);// 处理图片
                    if (picPath != null) {
                        this.uploadPic(storyInfo.getId(), picPath);
                    }
                }

                // 第一次抓取失败的任务，这再再次启动任务
                List<StoryDetail> storyDetailList = storyDetailService.queryByStoryIdAndContentIsNull(spiderStoryJob
                        .getStoryId());
        ...
        if (StringUtils.isNotEmpty(template.getNextXPath())) {
            ...
            if (nextAnchorList != null && nextAnchorList.size() > 0) {
                try {
                    ...
                    if (StringUtils.isNotEmpty(template.getReqeustEncode())) {
                        URL nextUrl = nextHtmlPage.getWebResponse().getRequestSettings().getUrl();
                        WebRequestSettings webRequest = new WebRequestSettings(nextUrl);
                        webRequest.setCharset(template.getReqeustEncode());
                        nextHtmlPage = webClientPool.borrowWebClient().getPage(webRequest);
                    } else {
    ...
    // 分析小说明细入口,count已入库条数
    private void parseDetailAnchor(List<Anchor> anchorList, ParseStoryElement parseStoryElement, StoryInfo storyInfo,
    ...
    public boolean isStoryFinish(StoryDetail storyDetail) {
        if (storyDetail != null && storyDetail.getTitle() != null) {
            ...
            if (title.indexOf("大结局") != -1 || title.indexOf("完本感言") != -1) {
                return true;
            }
        }
    ...
    public static void uploadPic(int storyId, String picUrl) {
        try {
            String url = SpiderConfigs.IMG_DOAMIN + SpiderConfigs.IMG_UPLOAD_QUERY;
            String hex = Hex.str2HexStr(picUrl);
            hex = hex.replace(" ", "");
            url = String.format(url, storyId, hex);
            CustomerHttpClient.simpleGet(url, ClientName.DEFAULT_CLIENT);
        } catch (ClientProtocolException e) {
            LOG.error("Upload story pic is error.", e);
        } catch (IOException e) {
            LOG.error("Upload story pic is error.", e);
        }
    }
    ...
    public static void main(String[] args){
        uploadPic(7, "687474703A2F2F696D6167652E73756E696E672E636E2F75696D672F6232632F6E6577636174656E74726965732F303037303037343435332D3030303030303030303135343231303335305F315F313630783136302E6A7067");
    }
}
