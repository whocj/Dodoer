...
<form id="pageForm" name="pageForm" method="post" action="${base }/sys/notice/list.htm">
<input type="hidden" name="pageNo" value="${page.pageIndex }"/>
	<table class="pn-ltable" width="100%" cellspacing="1" cellpadding="0" border="0">
		...
		<td>${datas.creator }</td>
		<td>
			<a href="${base }/sys/notice/edit.htm?id=${datas.id}" class="pn-opt">修改</a> |
			<a href="${base }/sys/notice/delete.htm?ids=${datas.id}" onclick="if(!confirm('你确定要删除吗?')) {return false;}" class="pn-opt">删除</a>
		</td>
		</tr>
		</#list>
		</tbody>
