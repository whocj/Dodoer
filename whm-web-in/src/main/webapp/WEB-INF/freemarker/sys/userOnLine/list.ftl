...
<title>在线用户</title> <#include "/common/include.ftl"/>
<script type="text/javascript">
	...
	function optDelete() {
		...
		var f = getTableForm();
		f.action = "${base}/sys/userOnLine/delete.htm";
		f.submit();
	}
</script>
</head>
<body>
	<div class="body-box">
		<div class="rhead">
			<div class="rpos">当前位置： 板块 - 在线用户 - 列表</div>
			<form class="ropt">
		...
		<form id="pageForm" name="pageForm" method="post"
			action="${base }/sys/userOnLine/list.htm">
			...
			<table class="pn-ltable" width="100%" cellspacing="1" cellpadding="0"
				...
				<tbody class="pn-ltbody">
					...
					<tr>
						...
						<td>${datas.loginIP }</td>
						<td><a
							href="${base }/sys/userOnLine/delete.htm?ids=${datas.id}"
							onclick="if(!confirm('你确定要删除吗?')) {return false;}" class="pn-opt">操作</a>
						</td>
					</tr>
					</#list>
				</tbody>
