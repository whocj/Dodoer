...
function optDelete() {
	...
	var f = getTableForm();
	f.action="${base}/spider/crawInfo/delete.htm";
	f.submit();
}
</script>
</head>
...
<div class="body-box">
<div class="rhead">
	<div class="rpos">当前位置： 板块 - 爬虫信息 - 列表</div>
	<form class="ropt">
		&nbsp;<input type="button" value="删除" onclick="optDelete();"/>
	</form>
	<div class="clear"></div>
</div>
<form id="pageForm" name="pageForm" method="post" action="${base }/spider/crawInfo/list.htm">
<input type="hidden" name="pageNo" value="${page.pageIndex }"/>
	<table class="pn-ltable" width="100%" cellspacing="1" cellpadding="0" border="0">
		...
		<td>
			<a href="${base }/spider/crawInfo/delete.htm?ids=${datas.id}" onclick="if(!confirm('你确定要删除吗?')) {return false;}" class="pn-opt">删除</a>
		</td>
		</tr>
		</#list>
		</tbody>
